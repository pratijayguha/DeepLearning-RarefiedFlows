...
save_post_dir = 'PostProc/Pressure/HPT_Pressure_hl/'

save_model_dir = 'SavedModels/Pressure/HPT_Pressure_hl/'
...
def load_optimizers(): # Function to load all optimizers
    Adadelta = tf.keras.optimizers.Adadelta(learning_rate=0.0001,
...
for i in range(len(opti_list)):
    ...
    def build_model(archi):
        ...
        for i, node in enumerate(archi):
            if i==0: # First layer in MaxVal fork
                layers.append(Dense(node, name='layers%d' %(i+1), activation='relu')(Input_layer))
                ...
                layers.append(Dense(node, name='layers%d' %(i+1), activation='relu')(layers[i-1]))
    ...
    keras.utils.plot_model(model, "Pressure_Architecture_HPT_hl.png", show_shapes=True)
    ...
    # plt.show(fig1)
    plt.savefig(save_post_dir + 'HPT_hl_stats.png', dpi=500)
    ...
    plt.savefig(save_post_dir + 'cumilative_ind_HPT_hl_predictions.png', dpi=500)
    ...
    for j in range(int(X_pred.shape[0]/201)):
        ...
        max_MAPE[i,2*j] = max_APE_temp[0]*10
        0
